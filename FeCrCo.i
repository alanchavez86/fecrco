...
# Koyama 2004, Table 1: kappa_c = 1.0e-14 (J*m^2/mol)
  ...
  [RT_clnc] # Koyama 2004, Second term of G^alpha_c (Equation 2 contribution of the free energy)
    ...
    constant_expressions ='${tempe}  6.24150934e+18  1e-27  8.31446261815324'
  ...
  [heat_of_mixing] # Koyama 2004, Third term of G^alpha_c (Equation 2 contribution of the free energy)
    ...
    constant_expressions ='6.24150934e+18  1e-27  ${tempe}'
    material_property_names = 'c1(c2,c3)'
  ...
  [magnetic_contribution_to_Gibbs] # Koyama 2004, Equation 2: ^mgG^alpha
    ...
    constant_expressions ='6.24150934e+18  1e-27  ${tempe}  8.31446261815324'
  ...
  [external_mag_energy]
    type = DerivativeParsedMaterial
    f_name = m_ext
    material_property_names = 'i_tau(c2,c3) c1(c2,c3)'
    constant_names = '     u     eV_J            d    B   H'
    constant_expressions ='1.25663706212e-6  6.24150934e+18  1e-27  9.27e-24    ${emag}'
    function = 'eV_J*d*(-u*H*B*(2.22*c1 - 0.01*c2 + 1.35*c3 - 0.85*c1*c2 + (2.4127 + 0.2418*(c3-c1))*c1*c3))*i_tau'
    args = 'c2 c3'
    outputs = exodus
    derivative_order = 2
  []
  ...
  [tau] # Koyama 2004, Equation 2: tau = T/Curie_Temperature
    ...
    f_name = tau
    material_property_names = 'c1(c2,c3)'
    function = '${tempe}/(1043*c1 - 311.5*c2 + 1450*c3 + (1650 + 550*(c2-c1))*c1*c2 + 590*c1*c3)'
    args = 'c2 c3'
    outputs = exodus
    derivative_order = 2
  []
  ...
  [brillouin_function] # Koyama 2004, I(tau)
    type = DerivativeParsedMaterial
    f_name = i_tau
    function = 'if(tau>0.9, 2^(-1*(2+10*(tau-1))), 1-1/7*(5*tau^4+2*tau^20))'
    args = 'c2 c3'
    material_property_names = 'tau(c2,c3) c1(c2,c3)'
    derivative_order = 2
  []
  ...
  [Mobility_M22] # Koyama 2004, After Equation 8: L22
    ...
    constant_expressions ='1e+09  6.24150934e+18  1e-27  ${tempe}  8.31446261815324'
    material_property_names = 'c1(c2,c3)'
    function = 'nm_m^2/eV_J/d*(c1*c2*(1.0e-4*exp(-294000/(8.31446261815324*T))) + (1-c2)^2*(2.0e-5*exp(-308000/(8.31446261815324*T))) + c2*c3*(1.0e-4*exp(-294000/(8.31446261815324*T))))*c2/(R*T)'
    args = 'c2 c3'
    derivative_order = 1
  []
  ...
  [Mobility_M23] # Koyama 2004, After Equation 8: L23
    ...
    constant_expressions ='1e+09  6.24150934e+18  1e-27  ${tempe}  8.31446261815324'
    material_property_names = 'c1(c2,c3)'
    function = 'nm_m^2/eV_J/d*(c1*(1.0e-4*exp(-294000/(8.31446261815324*T))) - (1-c2)*(2.0e-5*exp(-308000/(8.31446261815324*T))) - (1-c3)*(1.0e-4*exp(-294000/(8.31446261815324*T))))*c2*c3/(R*T)'
    args = 'c2 c3'
    derivative_order = 1
  []
  ...
  [Mobility_M33] # Koyama 2004, After Equation 8: L33
    ...
    constant_expressions ='1e+09  6.24150934e+18  1e-27  ${tempe}  8.31446261815324'
    material_property_names = 'c1(c2,c3)'
    function = 'nm_m^2/eV_J/d*(c1*c3*(1.0e-4*exp(-294000/(8.31446261815324*T))) + c2*c3*(2.0e-5*exp(-308000/(8.31446261815324*T))) + (1-c3)^2*(1.0e-4*exp(-294000/(8.31446261815324*T))))*c3/(R*T)'
    args = 'c2 c3'
    derivative_order = 1
  []
  ...
  [F_system] # Total free energy of the system
    ...
    f_name = F_sum
    sum_materials = 'mg_G RT_clnc E_G m_ext' #ElasticCr'
    args = 'c2 c3'
    derivative_order = 2
  []
[]
